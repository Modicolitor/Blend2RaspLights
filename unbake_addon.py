...
class UnBakeA(bpy.types.Operator):
    ...
    RGB = BoolProperty(name='Use RGB colors', default=True,
                       description='Use RGB colors for f-curves')
    ...
    def execute(self, context):
        obj = context.object
        pts = [(c.sampled_points, c.data_path, c.array_index)
               for c in obj.animation_data.action.fcurves if c.sampled_points and c.select]
        if pts:
...
class UnBakeB(bpy.types.Operator):
    ...
    scaleX = FloatProperty(name='X scale', min=.01, max=5,
                           default=.1, description='X scale for 3d curve')
    scaleY = FloatProperty(name='Y scale', min=.001, max=5,
                           default=1, description='Z scale for 3d curve')
    use_radius = BoolProperty(name='Use Radius', default=True,
                              description='Use Radius and Beveling or just Draw the Profile')
